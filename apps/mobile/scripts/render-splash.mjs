/**
 * Writes assets/splash.png — AppLogo (showBackground=false): transparent BG,
 * faint inner ring, gradient 300° arc, white dot. Run:
 *   pnpm --filter @focusflow/mobile exec node scripts/render-splash.mjs
 */
...
const outPath = path.join(mobileRoot, "assets", "splash.png");
/** Android native splash uses @mipmap/ic_launcher_foreground (same as app icon) — see res/drawable/splashscreen.xml */
...
/** Arc from 60° to 360° (300° sweep); gap is 0° < θ < 60° */
function angleOnArcDeg(deg) {
  if (deg >= 60) return true;
  return deg === 0;
}

/** Gradient parameter along arc: 0 at 60°, 1 at 360°/0° */
...
function blend(png, x, y, r, g, b, a) {
  ...
  const na = aa + oa * (1 - aa);
  if (na <= 0) return;
  png.data[i] = (r * aa + png.data[i] * oa * (1 - aa)) / na;
  png.data[i + 1] = (g * aa + png.data[i + 1] * oa * (1 - aa)) / na;
  png.data[i + 2] = (b * aa + png.data[i + 2] * oa * (1 - aa)) / na;
